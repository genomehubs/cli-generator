# Phase XX: Positional Hybrid (Local + Remote Oxford / Ribbon / Painting)

**Depends on:** Phase 11 (positional endpoint), Phase 12 (PlotSpec), Phase 13 (local report infrastructure)
...
This is a **client-side (SDK) concern**, not a server concern. The hybrid workflow is:

1. Parse local file → extract feature positions (SDK-side, via Rust through PyO3/WASM/extendr)
2. Fetch remote features via the existing `/api/v3/positional` endpoint (Phase 11)
3. Join both feature sets by shared marker identifier (group_by field)
4. Compute layout using the shared `positional_from_features()` Rust function
...
Status filter: include only `Complete` and `Duplicated` features by default.
`Duplicated` features are deduplicated by taking the highest-score instance.
`Fragmented` and `Missing` are excluded.
...
A user-supplied two-column TSV (`sequence_id\tlength`, no header):
...
This is always an underestimate (the last marker rarely sits at the chromosome tip):
...
## `positional_from_features()` and `hybrid_positional()`

Two pure Rust functions (no HTTP, no ES) in `crates/genomehubs-query/src/report/hybrid.rs`.
...
### Layout algorithm (from Phase 11)

The layout algorithm shared between Phase 11 (server) and this phase (client):

1. **`order_sequences_by_median()`** — for each assembly, order sequences by the median
   position of shared markers (so that syntenic chromosomes appear vertically aligned)
2. **`orient_sequence()`** — flip a sequence's strand if more markers match in the
   reverse orientation than the forward orientation
3. **`compute_offsets()`** — given ordered sequences and lengths, produce a cumulative
   offset map for scaling positions to plot coordinates
...
crates/genomehubs-query/src/parse_local/
    ...
    fai.rs          — samtools .fai index parser
...
crates/genomehubs-query/src/report/
    hybrid.rs       — positional_from_features() and hybrid_positional()
```
...
The approach here **does not depend on blobtk** as an upstream crate:
...
this crate in future, that is a separate decision.
