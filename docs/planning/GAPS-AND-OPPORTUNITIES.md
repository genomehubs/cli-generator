...
### 1. **Integration Runbook** ✅ COMPLETE

**Status**: [integration-runbook.md](./integration-runbook.md) — Step-by-step guide for onboarding any new repo to use cli-generator.

**Coverage**:

- Prerequisites (Rust, Python, Node, R setup verification)
- Key decisions: commit vs. CI-generate, versioning strategy
- 8-step workflow: config → generate → test → integrate → version → publish → monitor API changes → CI setup
- Testing for all languages (Python pytest, JavaScript npm test, R CMD check)
- Live API integration testing
- Two complete worked examples: boat-cli (CLI + SDKs) and assessment-api (SDKs only)
- Troubleshooting (7 common issues + solutions)
- Checklists for both Maven and subsequent sites

**Next Step**: Use this runbook to test boat-cli integration post-MVP

---

### 2. **Extension Guide** ✅ COMPLETE

**Status**: [extension-guide.md](./extension-guide.md) — Comprehensive pattern-based guide for extending cli-generator.

**Coverage**:

- **Rust-first pattern**: Logic in Rust core, expose via PyO3 + templates (no code duplication across languages)
- **5 worked examples**: New parameter, new language (Go), validator rule, snippet language, custom structure
- **Anti-patterns spelled out**: What NOT to do (duplicate logic in templates, implement separately in R/JS, create fallbacks)
- **Task checklist**: Verification steps before submitting (tests, docs, parity across languages)
- **Cross-language testing**: How to verify behavior identical in Python, R, JavaScript
- **Template lookup reference**: Where to find each customizable component

**Key learnings encoded**:

- validate + exclude filters pattern → DRY principle emphasized
- PyO3 checklist for 6 touch-points (nothing missed)
- Common pitfalls from path resolution to signature drift

**Next Step**: Use this guide for post-MVP feature additions
